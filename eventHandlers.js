...
exports.registerEventHandlers = function (source) {
    ...
    source.addEventListener('Trainingwiederholungen', handleTrainingwiederholungen);
    source.addEventListener('Trainingkadenz', handleTrainingkadenz);
    source.addEventListener('MyEvent', handleMyEvent);
...
function handleTrainingwiederholungen(event) {
    // read variables from the event
    var data = {
        eventName: event.type,
        eventData: JSON.parse(event.data).data, // the value of the event
    ...
    try {        
        ...
        // Log the event in the database
        logger.logOne("MyDB", "Trainingwiederholungen", data);
...
function handleTrainingkadenz(event) {
    // read variables from the event
    var data = {
        eventName: event.type,
        eventData: JSON.parse(event.data).data, // the value of the event
    ...
    try {        
        ...
        // Log the event in the database
        logger.logOne("MyDB", "Trainingkadenz", data);
